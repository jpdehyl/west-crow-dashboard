...
async function analyzeWithClaude(
  ...
  contentBlocks.push({
    ...
    text: [
      ...
      ``,
      `Analyze these demolition/construction documents. Your PRIMARY job is to MEASURE and EXTRACT quantities yourself from the drawings — do NOT ask JP for dimensions or areas you can read from the plans.`,
      ``,
      `MEASUREMENT RULES (mandatory):`,
      `- Floor areas: read from demo plan keynotes, room schedules, or calculate from dimensions shown. Never ask "what is the floor area?"`,
      `- Wall LF: count from demo plan. Use wall height from RCP (check keynotes carefully — mall retail is typically 9–14 ft, NOT 8 ft). SF = LF × height.`,
      `- Ceiling type and area: read from RCP. ACT = suspended tile grid. GWB = drywall ceiling. Split by type. Never assume 8 ft.`,
      `- If a dimension is shown on a drawing, USE IT. If a keynote says "demo existing" over an area, measure that area.`,
      `- Storefront LF: count from demo plan or elevation drawings.`,
      `- Only add to questions[] things that are genuinely IMPOSSIBLE to determine from the documents (e.g. hidden conditions, hazmat confirmation).`,
      `- DO NOT ask about quantities, areas, or dimensions that can be read from the drawings.`,
      ``,
      `For anything you cannot determine with confidence after carefully reading all documents, add it to a questions array.`,

    ...
    <div style={{ maxWidth: "1100px" }}>
      ...
      <div style={{ display: "flex", gap: "0", border: "1px solid var(--border)", borderRadius: "10px", overflow: "hidden", marginBottom: "2rem" }}>
        {STAGES.map(({ key, label }, i) => {
          ...
          return (
            <div key={key} style={{
              flex: 1, padding: "1rem 1.25rem", background: "var(--bg)",
              ...
              {sorted.map((bid, i) => {
                ...
                return (
                  <tr key={bid.id} className="row-hover">
                    ...
                    <td style={{ padding: "0.9rem 1.5rem", borderBottom: border }}>
            ...
            return (
              <div key={col.key}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.75rem", padding: "0 0.25rem" }}>
                  ...
                  <span style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--ink-muted)" }}>
                    {col.label}
                  </span>
                  <span style={{ fontSize: "11px", color: "var(--ink-faint)", marginLeft: "auto" }}>{colBids.length}</span>
                </div>
                {colValue > 0 && (
                  <p style={{ fontSize: "11px", color: "var(--ink-faint)", marginBottom: "0.75rem", padding: "0 0.25rem" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {colBids.length === 0 ? (
                    <div style={{ padding: "1.5rem", border: "1px dashed var(--border)", borderRadius: "10px", textAlign: "center", color: "var(--ink-faint)", fontSize: "12px" }}>
                      None
                    </div>
                  ) : colBids.map(bid => {
                    const days = daysUntil(bid.deadline)
                    ...
                    return (
                      <Link key={bid.id} href={`/bids/${bid.id}`} style={{ textDecoration: "none" }}>
                        <div className="row-hover" style={{
                          ...
                          <p style={{ fontSize: "13px", fontWeight: 600, color: "var(--ink)", marginBottom: "0.3rem", lineHeight: 1.3 }}>
                            {bid.project_name}
                          </p>
                          <p style={{ fontSize: "11px", color: "var(--ink-faint)", marginBottom: "0.85rem" }}>
                            {bid.client}
                          </p>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <span style={{ fontFamily: "var(--font-serif), serif", fontSize: "1rem", color: "var(--ink)", letterSpacing: "-0.02em" }}>
                              {formatCurrency(bid.bid_value)}
                            </span>
                            {urgent ? (
                              <span style={{ fontSize: "11px", background: "var(--terra-light)", color: "var(--terra)", padding: "2px 7px", borderRadius: "4px", fontWeight: 600 }}>
                                {days}d
                              </span>
                            ) : (
                          ...
                          {bid.margin_pct != null && (
                            <div style={{ marginTop: "0.5rem", fontSize: "11px", color: "var(--sage)", fontWeight: 500 }}>
                              {bid.margin_pct}% margin ✓
                            </div>
                          )}
                        </div>
                      </Link>

...
function SectionMarker({ n, title, date }: { n: string; title: string; date?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "1rem", margin: "3rem 0 1.75rem" }}>
      <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.18em", color: "var(--terra)", fontFamily: "var(--font-sans), sans-serif", flexShrink: 0 }}>
        {n}
      </span>
      <div style={{ flex: 1, height: "1px", background: "var(--border)" }} />
      <span style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--ink-muted)", flexShrink: 0 }}>
        {title}
      </span>
      {date && (
        <>
          <div style={{ width: "1px", height: "12px", background: "var(--border)" }} />
          <span style={{ fontSize: "11px", color: "var(--ink-faint)", flexShrink: 0 }}>{date}</span>
        </>
      )}
    </div>
  )
...
function StoryCard({ children, accent }: { children: React.ReactNode; accent?: string }) {
  return (
    <div style={{
      ...
      borderLeft: `3px solid ${accent || "var(--terra)"}`,
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "0.55rem 0", borderBottom: "1px solid var(--border)" }}>
      ...
      <span style={{
        ...
        fontFamily: mono ? "var(--font-serif), serif" : "inherit",
...
export default function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  const costByCat = project.costs.reduce((acc, c) => {
    acc[c.category] = (acc[c.category] || 0) + c.amount; return acc
  }, {} as Record<string, number>)

  const budgetByCat: Record<string, number> = {
    labour:         project.budget_labour,
    materials:      project.budget_materials,
    equipment:      project.budget_equipment,
    subcontractor:  project.budget_subs,
  }

  // ── Timeline ─────────────────────────────────────────────────────────────
  const bidInvited    = bid?.timeline.find(e => e.stage === 'invited')
  const bidEstimated  = bid?.timeline.find(e => e.stage === 'estimating')
  const bidSubmitted  = bid?.timeline.find(e => e.stage === 'submitted')
  const bidWon        = bid?.timeline.find(e => e.stage === 'decision')
  const invoiceSent   = project.invoices?.find(i => i.sent_date)
  const invoicePaid   = project.invoices?.find(i => i.paid_date)
  ...
  const currentStageIdx = LIFECYCLE.reduce((last, s, i) => s.done ? i : last, 0)
  ...
  // ── Invoice summary ───────────────────────────────────────────────────────
  const totalInvoiced = project.invoices?.reduce((s, i) => s + i.gross_amount, 0) || 0
  const totalPaid     = project.invoices?.filter(i => i.paid_date).reduce((s, i) => s + i.gross_amount, 0) || 0
  ...
  return (
    <div style={{ maxWidth: "820px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem" }}>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: "11px", letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--ink-faint)", fontWeight: 500, marginBottom: "0.4rem" }}>
              {project.client} · {project.estimator}
              {project.superintendent && ` · Oscar (super)`}
            </p>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", alignItems: "center" }}>
              ...
              {notStarted ? (
                ...
                <span style={{ fontSize: "12px", fontWeight: 600, color: "var(--sage)", background: "var(--sage-light)", padding: "2px 9px", borderRadius: "5px" }}>
                  ● Active
                </span>
              )}
            </div>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            <p style={{ fontFamily: "var(--font-serif), serif", fontSize: "2rem", fontWeight: 400, letterSpacing: "-0.04em", color: "var(--ink)" }}>
          ...
          {LIFECYCLE.map((stage, i) => {
            ...
            return (
              <div key={stage.key} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative" }}>
                ...
                {!isLast && (
                  <div style={{
                    position: "absolute", top: "9px", left: "50%", width: "100%", height: "2px",
                    background: i < currentStageIdx ? "var(--terra)" : "var(--border)",
                    zIndex: 0,
                  }} />
                )}
                {/* Dot */}
                <div style={{
                  ...
                  background: isDone ? "var(--terra)" : "var(--border)",
                  border: isActive ? "3px solid var(--terra)" : "none",
                  ...
                  {isActive && (
                    <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--terra)", display: "block" }} />
                  )}
                </div>
                <p style={{ fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", marginTop: "0.4rem", color: isDone ? "var(--ink)" : "var(--ink-faint)", textAlign: "center" }}>
                  {stage.label}
                </p>
      ...
      {!notStarted && (
        <div style={{ marginBottom: "2.5rem" }}>
          <div style={{ height: 4, background: "var(--border)", borderRadius: 2, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${timePct}%`, background: "var(--gold)", borderRadius: 2, transition: "width 0.3s ease" }} />
          </div>
          ...
          <StoryCard>
            ...
            {bid.documents && bid.documents.length > 0 && (
              <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid var(--border)" }}>
                <p style={{ fontSize: "11px", letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--ink-faint)", marginBottom: "0.6rem" }}>
                  Documents received ({bid.documents.length})
                </p>
      ...
      {bid && bidEstimated && (
        <>
          <p style={{ fontSize: "14px", color: "var(--ink-muted)", lineHeight: 1.75, marginBottom: "1.25rem" }}>
            <strong style={{ color: "var(--ink)", fontWeight: 600 }}>{bid.estimator}</strong> completed the takeoff and estimate
            on <strong style={{ color: "var(--ink)", fontWeight: 600 }}>{formatDate(bidEstimated.date)}</strong>.
      ...
      {bid && bidWon && (
        <>
          <p style={{ fontSize: "14px", color: "var(--ink-muted)", lineHeight: 1.75, marginBottom: "1.25rem" }}>
            ...
            {bid.margin_pct ? ` Estimated margin locked at ${bid.margin_pct}%.` : ""}
          </p>
          <StoryCard accent="var(--sage)">
            ...
            {bidWon.by && <KVRow label="Confirmed by" value={bidWon.by} />}
          </StoryCard>
        </>
      )}

      {/* ══════════════════════════════════════════════════════════════════════
          SECTION 04 · FIELD EXECUTION
      ══════════════════════════════════════════════════════════════════════ */}
      ...
      {notStarted ? (
        <div style={{ padding: "2rem", background: "var(--gold-light)", border: "1px solid var(--border)", borderRadius: "10px", textAlign: "center" }}>
          <p style={{ fontSize: "14px", color: "var(--ink-muted)" }}>
            Mobilization begins <strong style={{ color: "var(--ink)" }}>{formatDate(project.start_date)}</strong>.
            Daily logs will appear here once work is underway.
          </p>
        </div>
      ) : (
        <>
          <p style={{ fontSize: "14px", color: "var(--ink-muted)", lineHeight: 1.75, marginBottom: "1.25rem" }}>
            <strong style={{ color: "var(--ink)", fontWeight: 600 }}>
              {project.superintendent || project.estimator}
            </strong>{" "}
            mobilized on{" "}
            <strong style={{ color: "var(--ink)", fontWeight: 600 }}>{formatDate(project.start_date)}</strong>.{" "}
            {project.daily_logs.length} day{project.daily_logs.length !== 1 ? "s" : ""} logged so far · {project.daily_logs.reduce((s,d) => s + d.hours, 0)} total hours on site.
          </p>

          {/* Daily logs — field report format */}
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {project.daily_logs.map((log, i) => {
              ...
              const wxIcon    = WEATHER_ICON[weather] || ""

              return (
                <div key={log.id} style={{
                  background: "var(--bg)",
                  border: "1px solid var(--border)",
                  borderLeft: log.issues ? "3px solid var(--terra)" : "3px solid var(--border)",
                  borderRadius: "0 10px 10px 0",
                  overflow: "hidden",
                }}>
                  {/* Log header */}
                  <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                      <span style={{ fontFamily: "var(--font-serif), serif", fontSize: "1rem", color: "var(--ink)", fontWeight: 400, letterSpacing: "-0.02em" }}>
                        {formatDateShort(log.date)}
                      </span>
                      <span style={{ fontSize: "11px", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--ink-faint)" }}>
                        Day {dayNum}
                      </span>
                      {wxIcon && (
                        <span style={{ fontSize: "12px", color: "var(--ink-faint)" }}>{wxIcon} {weather}</span>
                      )}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                      <span style={{ fontSize: "12px", color: "var(--ink-faint)" }}>{log.hours} hrs</span>
                      {log.photos && (
                        <span style={{ fontSize: "11px", color: "var(--ink-faint)" }}>📷 {log.photos}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ padding: "1rem 1.25rem" }}>
                    <p style={{ fontSize: "13px", color: "var(--ink)", lineHeight: 1.7, marginBottom: "0.6rem" }}>
                      {log.work_performed}
                    </p>
                    ...
                    {log.issues && (
                      <div style={{
                        ...
                        <p style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--terra)", marginBottom: "0.3rem" }}>
                          ⚠ Issue / Note
                        </p>
      ...
      <div style={{ background: "var(--bg-subtle)", border: "1px solid var(--border)", borderRadius: "10px", padding: "1.25rem 1.5rem", marginBottom: "1.25rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "1.25rem" }}>
          ...
          <p style={{ fontSize: "11px", color: "var(--ink-faint)" }}>
            {formatCurrency(totalCosts)} spent · {formatCurrency(budgetTotal - totalCosts)} remaining
          </p>
        </div>
            ...
            <div key={cat} style={{ marginBottom: "1rem" }}>
              ...
              <div style={{ height: 5, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
                <div style={{
                  ...
                  background: over ? "var(--terra)" : COST_COLOR[cat] || "var(--sage)",
          ...
          <tbody>
            {project.costs.map((cost, i) => (
              <tr key={cost.id} className="row-hover">
                <td style={{ padding: "0.75rem 1.25rem", borderBottom: i < project.costs.length-1 ? "1px solid var(--border)" : "none", color: "var(--ink-faint)", whiteSpace: "nowrap" }}>
                  {formatDateShort(cost.date)}
                </td>
                <td style={{ padding: "0.75rem 1.25rem", borderBottom: i < project.costs.length-1 ? "1px solid var(--border)" : "none", color: "var(--ink)", fontWeight: 500 }}>
                  {cost.description}
                </td>
                ...
                <td style={{ padding: "0.75rem 1.25rem", borderBottom: i < project.costs.length-1 ? "1px solid var(--border)" : "none", color: "var(--ink-faint)" }}>
                  {cost.vendor || "—"}
                </td>
                <td style={{ padding: "0.75rem 1.25rem", borderBottom: i < project.costs.length-1 ? "1px solid var(--border)" : "none", textAlign: "right", fontFamily: "var(--font-serif), serif", color: "var(--ink)", fontWeight: 500 }}>
                  {formatCurrency(cost.amount)}
                </td>
              </tr>
            ))}
            <tr style={{ background: "var(--bg-subtle)" }}>
              <td colSpan={4} style={{ padding: "0.75rem 1.25rem", textAlign: "right", fontSize: "12px", fontWeight: 600, color: "var(--ink-muted)", letterSpacing: "0.05em", textTransform: "uppercase", borderTop: "1px solid var(--border)" }}>
                Total spent
              </td>
              <td style={{ padding: "0.75rem 1.25rem", textAlign: "right", fontFamily: "var(--font-serif), serif", fontSize: "1.1rem", color: "var(--ink)", fontWeight: 600, borderTop: "1px solid var(--border)" }}>
      ...
      {/* ══════════════════════════════════════════════════════════════════════
          SECTION 06 · INVOICE & COLLECTIONS
      ══════════════════════════════════════════════════════════════════════ */}
      ...
      {(!project.invoices || project.invoices.length === 0) ? (
        <div style={{ padding: "1.5rem", background: "var(--bg-subtle)", border: "1px solid var(--border)", borderRadius: "10px", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "12px", color: "var(--ink-faint)", marginTop: "0.25rem" }}>Invoice will appear here once submitted to {project.client}.</p>
        </div>
      ) : (
            ...
            return (
              <div key={inv.id} style={{ border: "1px solid var(--border)", borderRadius: "10px", overflow: "hidden" }}>
                ...
                <div style={{
                  ...
                  background: isPaid ? "var(--sage-light)" : isSent ? "var(--gold-light)" : "var(--bg-subtle)",
                  borderBottom: "1px solid var(--border)",
                }}>
                  ...
                  <span style={{
                    ...
                    color: isPaid ? "var(--sage)" : isSent ? "var(--gold)" : "var(--ink-faint)",
                    padding: "2px 8px", borderRadius: "4px",
                    background: isPaid ? "rgba(90,122,90,0.1)" : isSent ? "rgba(196,150,58,0.1)" : "transparent",
                  }}>
                ...
                <div style={{ padding: "1.25rem 1.5rem" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 2rem" }}>
                    <div>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "0.65rem 0", borderBottom: "2px solid var(--ink)" }}>
                        ...
                        <span style={{ fontFamily: "var(--font-serif), serif", fontSize: "1.2rem", color: "var(--ink)", letterSpacing: "-0.02em", fontWeight: 400 }}>
                  ...
                  {inv.notes && (
                    <p style={{ fontSize: "12px", color: "var(--ink-faint)", marginTop: "1rem", lineHeight: 1.65, borderTop: "1px solid var(--border)", paddingTop: "0.75rem" }}>
                      {inv.notes}
                    </p>
                  )}
                </div>
      ...
      {bid && (
        <div style={{ marginTop: "3rem", padding: "1rem 1.5rem", background: "var(--bg-subtle)", borderRadius: "8px", border: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: "13px", color: "var(--ink-muted)" }}>
            View original bid — {formatCurrency(bid.bid_value)} · {bid.estimator}
          </p>
          <Link href={`/bids/${bid.id}`} style={{ fontSize: "12px", color: "var(--terra)", textDecoration: "none" }}>
            Open bid file →
          </Link>
        </div>
      )}
